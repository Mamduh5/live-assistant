...
CDP connection alone is `connecting`; only detection of an active selected Webcast socket is `connected`. An explicitly open socket remains healthy through arbitrary frame silence. Closing one selected socket does nothing while another remains; closing the last starts a bounded replacement-socket timer. A replacement cancels that timer and restores `connected` without navigation. Replacement timeout, target loss, discovery failure, and CDP loss dispose the owned session and retry with bounded jittered backoff. Shutdown closes only the owned target and local CDP connection and never sends `Browser.close`.
